...
def run_sanity_check(model, device, config, results_dir):
    print("\n--- STEP 3A: LSTM TRAINING SANITY CHECK ---", flush=True)
    ...
    sanity_md = []
    sanity_md.append("# B7 — LSTM TRAINING SANITY CHECK REPORT\n")
    ...
    with open(os.path.join(results_dir, "lstm_sanity_check.md"), "w", encoding="utf-8") as f:
        f.write("\n".join(sanity_md))
        
    if not sanity_pass:
        raise RuntimeError("LSTM Training Sanity Check Failed!")
...
def run_training_pipeline(base_dir=r"d:\My Projects\SIH2026_PersonB"):
    ...
    lstm_tot_params, lstm_trn_params = model.count_parameters()
    
    # Calculate GRU parameters for fair comparison report
    ...
    print("--- STEP 3B: RUNNING 2-EPOCH LSTM SMOKE TEST ---", flush=True)
    ...
    print(f"--- STEP 3C: FULL LSTM TRAINING RUN ({max_epochs} Max Epochs) ---", flush=True)
    ...
    # Save History CSV
    history_csv_path = os.path.join(results_dir, "lstm_training_history.csv")
    ...
    # Write Report
    report_lines = []
    report_lines.append("# B7 — LSTM TRAINING REPORT\n")
    report_lines.append("## Executive Summary\n")
    report_lines.append("Direct sequence-to-sequence LSTM baseline model trained for 72-hour continuous PM2.5 forecasting.\n")
    
    report_lines.append("## Model Architecture & Parameter Count Comparison\n")
    report_lines.append(f"- **Architecture:** `LSTMForecastModel` (2-Layer LSTM + Linear Projection)")
    report_lines.append(f"- **Input Size:** `49` features")
    report_lines.append(f"- **Sequence Length:** `72` hours")
    report_lines.append(f"- **Hidden Dimension:** `128` units")
    report_lines.append(f"- **LSTM Layers:** `2` layers (Dropout = `0.2`)")
    report_lines.append(f"- **Output Size:** `1` (PM2.5 prediction)")
    report_lines.append(f"- **LSTM Total Parameters:** `{lstm_tot_params:,}`")
    report_lines.append(f"- **LSTM Trainable Parameters:** `{lstm_trn_params:,}`")
    report_lines.append(f"- **GRU Total Parameters (B6 Baseline):** `{gru_tot_params:,}`")
    report_lines.append(f"- **Parameter Difference:** LSTM has `+55,936` parameters (+33.3% capacity due to 4 gates vs 3 gates)\n")
    ...
    report_lines.append(f"- **Best Checkpoint Path:** `models/checkpoints/lstm_best.pt`\n")
    ...
    report_md_path = os.path.join(results_dir, "lstm_training_report.md")
