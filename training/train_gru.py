...
def run_sanity_check(model, device, config, results_dir):
    print("\n--- STEP 13: TRAINING SANITY CHECK ---", flush=True)
    ...
    sanity_md = []
    sanity_md.append("# B6 — GRU TRAINING SANITY CHECK REPORT\n")
    ...
    with open(os.path.join(results_dir, "gru_sanity_check.md"), "w", encoding="utf-8") as f:
        f.write("\n".join(sanity_md))
        
    if not sanity_pass:
        raise RuntimeError("GRU Training Sanity Check Failed!")
...
def run_training_pipeline(base_dir=r"d:\My Projects\SIH2026_PersonB"):
    ...
    print("--- STEP 15: RUNNING 2-EPOCH SMOKE TEST ---", flush=True)
    ...
    print(f"--- STEP 16: FULL GRU TRAINING RUN ({max_epochs} Max Epochs) ---", flush=True)
    ...
    # Save History CSV
    history_csv_path = os.path.join(results_dir, "gru_training_history.csv")
    ...
    # Write Report
    report_lines = []
    report_lines.append("# B6 — GRU TRAINING REPORT\n")
    report_lines.append("## Executive Summary\n")
    report_lines.append("Direct sequence-to-sequence GRU baseline model trained for 72-hour continuous PM2.5 forecasting.\n")
    
    report_lines.append("## Model Architecture & Parameter Count\n")
    report_lines.append(f"- **Architecture:** `GRUForecastModel` (2-Layer GRU + Linear Projection)")
    ...
    report_lines.append(f"- **GRU Layers:** `2` layers (Dropout = `0.2`)")
    report_lines.append(f"- **Output Size:** `1` (PM2.5 prediction)")
    report_lines.append(f"- **Total Parameters:** `{tot_params:,}`")
    report_lines.append(f"- **Trainable Parameters:** `{trn_params:,}`\n")
    ...
    report_lines.append(f"- **Best Checkpoint Path:** `models/checkpoints/gru_best.pt`\n")
    
    report_lines.append("## Training Progression (First 5 & Best Epochs)\n")
    ...
    report_md_path = os.path.join(results_dir, "gru_training_report.md")
